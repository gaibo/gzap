...
def make_basicstatstable(cashinstrs):
    """ Make visually formatted stats table from GZAP CashInstr (or multiple CashInstr)
    :param cashinstrs: single or multiple CashInstr
    :return: visually formatted pd.DataFrame
    """
    if isinstance(cashinstrs, CashInstr):
        cashinstrs = [cashinstrs]     # Convert single object to 1-element list for consistency
    elif not isinstance(cashinstrs, Iterable):
        raise ValueError("Argument <cashinstrs> must be an CashInstr or collection of CashInstr")
    # Create and collect table rows with details from each CashInstr
    cashinstr_table_list = []
    for cashinstr in cashinstrs:
        prices = cashinstr.price()
        pct_changes = cashinstr.price_return(logarithmic=False)
        cashinstr_table = \
            pd.concat([pd.DataFrame({cashinstr.name: [prices.first_valid_index(),
                                                      prices.last_valid_index()]},
                                    index=['start date', 'end date']).T,
                       prices.describe().rename(cashinstr.name).iloc[0:1].to_frame().T.astype(int),
                       prices.describe().rename(cashinstr.name).iloc[1:].to_frame().T,
                       pct_changes.describe().rename(cashinstr.name).iloc[1:].to_frame().T
                       .add_suffix(' change (%)') * 100],
                      axis=1)
        cashinstr_table_list.append(cashinstr_table)
    basicstatstable = pd.concat(cashinstr_table_list)   # Concat into final table
...
def main():
    ...
    vix = VolatilityIndex(bbg_data[('VIX Index', 'PX_LAST')], spx, name='VIX')
    ty1 = Index(bbg_data[('TY1 Comdty', 'PX_LAST')], name='TY1')
    tyvix = VolatilityIndex(bbg_data[('TYVIX Index', 'PX_LAST')], ty1, name='TYVIX')
    jb1 = Index(bbg_data[('JB1 Comdty', 'PX_LAST')], name='JB1')
    jgbvix = VolatilityIndex(bbg_data[('SPJGBV Index', 'PX_LAST')], jb1, name='JGB VIX')
    sx5e_tr = Index(bbg_data[('SX5E Index', 'TOT_RETURN_INDEX_GROSS_DVDS')], name='Euro Stoxx 50')
    agg_tr = ETF(bbg_data[('AGG Equity', 'TOT_RETURN_INDEX_GROSS_DVDS')], name='AGG')
    hyg_tr = ETF(bbg_data[('HYG Equity', 'TOT_RETURN_INDEX_GROSS_DVDS')], name='HYG')
    ief_tr = ETF(bbg_data[('IEF Equity', 'TOT_RETURN_INDEX_GROSS_DVDS')], name='IEF')
    lqd_tr = ETF(bbg_data[('LQD Equity', 'TOT_RETURN_INDEX_GROSS_DVDS')], name='LQD')
    ...
    make_regime(vix.vol_regime()[2], 'High Vol Regime', 'Date', 'Index Level', 'VIX Vol Regimes', ax=ax, color='r')
    make_regime(vix.vol_regime()[1], 'Low Vol Regime', 'Date', 'Index Level', 'VIX Vol Regimes', ax=ax, color='g')
